...
//-------------------------------------------
{
	...
	if ( pm->ps->weapon == WP_DISRUPTOR && pm->ps->weaponstate == WEAPON_READY )
	{
			...
			pm->cmd.upmove <= 0 && !pm->cmd.forwardmove && !pm->cmd.rightmove*/)
		{
			...
			if ( !pm->ps->zoomMode && pm->ps->pm_type != PM_DEAD && !(g_entities[pm->ps->clientNum].client && g_entities[pm->ps->clientNum].client->account && (g_entities[pm->ps->clientNum].client->account->flags & ACCOUNTFLAG_BOOST_AIMBOTBOOST) && g_aimbotBoost_hitscan.integer))
			{
