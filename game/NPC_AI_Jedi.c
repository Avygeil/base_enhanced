...
void WP_ResistForcePush( gentity_t *self, gentity_t *pusher, qboolean noPenalty )
{
	...
	if ( ((self->s.number < MAX_CLIENTS)|| self->client->NPC_class == CLASS_DESANN || !Q_stricmp("Yoda",self->NPC_type) || self->client->NPC_class == CLASS_LUKE || self->client->NPC_class == CLASS_KYLE)
...
void Boba_FireFlameThrower( gentity_t *self )
{
	...
	{
		G_Damage( traceEnt, self, self, dir, tr.endpos, damage, DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK|DAMAGE_IGNORE_TEAM, MOD_LAVA );
...
evasionType_t Jedi_SaberBlockGo( gentity_t *self, usercmd_t *cmd, vec3_t pHitloc, vec3_t phitDir, gentity_t *incoming, float dist ) //dist = 0.0f
{
	...
	{//jump!
		...
		{//gotta jump!
			...
			{
				if ( d_slowmodeath.integer > 5 && self->enemy && self->enemy->s.number < MAX_CLIENTS )
				{
...
static qboolean Jedi_SaberBlock( int saberNum, int bladeNum ) //saberNum = 0, bladeNum = 0
{
	...
	trap_Trace( &tr, saberPoint, saberMins, saberMaxs, hitloc, NPC->enemy->s.number, CONTENTS_BODY );
...
static qboolean Jedi_AttackDecide( int enemy_dist )
{
	...
	{//we won a saber lock, press the advantage with an attack!
		...
		if ( NPC->client->NPC_class == CLASS_DESANN || NPC->client->NPC_class == CLASS_LUKE 
			|| NPC->client->NPC_class == CLASS_KYLE || !Q_stricmp("Yoda",NPC->NPC_type) )
		{//desann and luke
			chance = 20;
		}
