...
extern vmCvar_t     g_npc_spawn_limit;
...
gentity_t *NPC_Spawn_Do( gentity_t *client_ent, gentity_t *ent )
{
...
*/
	...
	if ( newent == NULL ) 
	{
		trap_SendServerCommand( ent-g_entities, "print \""S_COLOR_RED"ERROR: NPC G_Spawn failed\n\"");
	...
	if ( newent->NPC == NULL ) 
	{		
		trap_SendServerCommand( ent-g_entities, "print \""S_COLOR_RED"ERROR: NPC G_Alloc NPC failed\n\"");
	...
	{
		trap_SendServerCommand( ent-g_entities, "print \""S_COLOR_RED"ERROR: NPC BG_Alloc client failed\n\"");
...
//==NPC_Connect( newent, net_name );===================================
	...
	if ( Q_stricmp( ent->classname, "NPC_Vehicle" ) == 0 )
	{
		...
		switch( g_vehicleInfo[iVehIndex].type )
		{
			...
			default:
				trap_SendServerCommand( ent-g_entities, va("print \""S_COLOR_RED "ERROR: Couldn't spawn NPC %s\n\"", ent->NPC_type));
...
//=====================================================================
	...
	if (ent->NPC_targetname && *ent->NPC_targetname){
...
gentity_t *NPC_SpawnType( gentity_t *ent, char *npc_type, char *targetname, qboolean isVehicle ) 
{
	...
	//G_Printf("found empty spot for spawn: %i\n", NPCspawner - g_entities);
	...
	return (NPC_Spawn_Do( ent,  NPCspawner ));
}
...
void NPC_Spawn_f( gentity_t *ent ) 
{
	...
	//we check for limits first

	for ( i = 0, n = 0; i < ENTITYNUM_WORLD; i++ )
	{
		npc_ent = &g_entities[i];
		if ( npc_ent && npc_ent->client && npc_ent->s.eType==ET_NPC ){
			++n;
		}
	}

	if (n >= g_npc_spawn_limit.integer){
		trap_SendServerCommand( ent-g_entities, va("print \"NPC limit %i reached, unable to create more.\n\"",g_npc_spawn_limit.integer));
		return;
	}
...
void NPC_Kill_f( gentity_t *ent ) 
{
	...
	if ( !name[0] )
	{
		trap_SendServerCommand( ent-g_entities, "print \""S_COLOR_RED"Error, Expected:\n"S_COLOR_WHITE
			...
			"or\n"
			"NPC kill team '[teamname]' - kills all NPCs of a certain team ('nonally' is all but your allies)\n\""
			);
	...
	if ( Q_stricmp( "team", name ) == 0 )
	{
		...
		if ( !name[0] )
		{
			...
			trap_SendServerCommand( ent-g_entities, "print \"Valid team names are:\n\"");

			for ( n = (TEAM_FREE + 1); n < TEAM_NUM_TEAMS; n++ )
			{
				trap_SendServerCommand( ent-g_entities, va("print \"%s\n\"", TeamNames[n]));
			}
			trap_SendServerCommand( ent-g_entities, "print \"nonally - kills all but your teammates\n\"");
			return;
		}
...
void NPC_PrintScore( gentity_t *ent, gentity_t *npc_ent )
{
	trap_SendServerCommand( ent-g_entities, va("print \"%s: %d\n\"", npc_ent->targetname, npc_ent->client->ps.persistant[PERS_SCORE]));
}
...
void Cmd_NPC_f( gentity_t *ent ) 
{
	...
	else if ( Q_stricmp ( cmd, "list" ) == 0 )
	{
		...
		trap_SendServerCommand( ent-g_entities, "print \"NPC list:\n\"");
