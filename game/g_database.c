...
static int RecalculateAndRecacheStreaks(void) {
	...
	static const char *sqlStreaksQuery =
		"WITH "
		"base AS (                              "  /* raw rows           */
		"    SELECT                                    "
		"        s.account_id,                         "
		"        p.match_id,                           "
		"        p.datetime          AS played_at,     "
		"        CASE WHEN p.win_team = pptp.team       "
		"             THEN  1 ELSE -1 END  AS wl       "
		"    FROM   sessions              AS s         "
		"    JOIN   playerpugteampos pptp ON pptp.session_id = s.session_id "
		"    JOIN   pugs               p  ON p.match_id     = pptp.match_id "
		"    WHERE  s.account_id IS NOT NULL           "
		"      AND  p.win_team IN (1,2)                "
		"      AND NOT EXISTS (                        "  /* spectate-swap rows */
		"            SELECT 1                          "
		...
		"), "
		"dedup AS (                             "  /* one row per match  */
		"    SELECT * FROM (                           "
		"        SELECT  b.*,                          "
		"                ROW_NUMBER() OVER (           "
		"                    PARTITION BY b.account_id, b.match_id "
		...
		"    FROM dedup d                               "
		"), "
		"with_prev AS (                         "  /* previous W/L value */
		"    SELECT o.*,                                "
		"           LAG(o.wl) OVER (                    "
		...
		"    FROM ordered o                             "
		"), "
		"grp_calc AS (                          "  /* gaps-and-islands   */
		"    SELECT wp.*,                               "
		"           SUM(                                "
		"               CASE                            "
		"                   WHEN wp.prev_wl IS NULL     "
		"                        OR wp.prev_wl = wp.wl  "
		"                   THEN 0 ELSE 1               "
		"               END                             "
		"           ) OVER (                            "
		"               PARTITION BY wp.account_id      "
		"               ORDER BY     wp.rn_desc         "
		"           ) AS grp                            "
		"    FROM with_prev wp                          "
		") "
		"SELECT                                         "
		"    gc.account_id,                             "
		"    (CASE MAX(gc.wl) WHEN 1 THEN +1 ELSE -1 END) "
		"      * COUNT(*)            AS current_streak  "
		"FROM   grp_calc gc                             "
		"WHERE  gc.grp = 0              /* newest run */"
		"GROUP  BY gc.account_id;                       ";
