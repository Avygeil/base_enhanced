...
void G_UpdateNonClientBroadcasts( gentity_t *self ) {
	...
	if ( self->s.eType == ET_MISSILE && self->r.ownerNum >= 0 && self->r.ownerNum < MAX_CLIENTS ) {
		...
		self->r.broadcastClients[1] |= level.racemodeClientMask; // by default, just hide to racers and racespectators...
		self->r.broadcastClients[1] &= level.racemodeClientsHidingIngameMask; // ...but show to racers who didn't disable seeing in game stuff...
		self->r.broadcastClients[1] |= level.racemodeSpectatorMask; // ...and hide it to racespectators
