...
"WITH winplaytime AS ( "
"SELECT playerpugteampos.session_id, playerpugteampos.pos, CAST(sum(CASE WHEN team = win_team THEN playerpugteampos.duration ELSE 0 END) AS FLOAT) AS playtime "
...
"totalplaytime AS ( "
"SELECT playerpugteampos.session_id, playerpugteampos.pos, CAST(sum(playerpugteampos.duration) AS FLOAT) AS playtime "
"FROM playerpugteampos "
"JOIN sessions ON sessions.session_id = playerpugteampos.session_id "
"JOIN pugs ON pugs.match_id = playerpugteampos.match_id "
"GROUP BY account_id, playerpugteampos.pos) "
"SELECT accounts.account_id, accounts.name, playerpugteampos.pos, "
"winplaytime.playtime / totalplaytime.playtime AS winrate, "
"CAST (sum(json_extract(playerpugteampos.stats, '$.cap')) AS FLOAT) / (CAST (sum(playerpugteampos.duration) AS FLOAT) / 1200000) AS avg_cap "
"FROM playerpugteampos "
"JOIN sessions ON playerpugteampos.session_id = sessions.session_id "
"JOIN accounts ON accounts.account_id = sessions.account_id "
"JOIN pugs ON pugs.match_id = playerpugteampos.match_id "
"JOIN winplaytime ON winplaytime.session_id = sessions.session_id AND winplaytime.pos = playerpugteampos.pos "
"JOIN totalplaytime ON totalplaytime.session_id = sessions.session_id AND totalplaytime.pos = playerpugteampos.pos "
